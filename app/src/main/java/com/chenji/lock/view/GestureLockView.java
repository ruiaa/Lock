...
public class GestureLockView extends View {
	private static final int ANIM_DURATION = 200;// 点选中时候的动画时间
	private static final int RECOVER_DURATION = 1000;// 错误路径持续时长

	private static final int RIGHT_COLOR= Util.getColor(R.color.password_right);       //2
	private static final int ERROR_COLOR=Util.getColor(R.color.password_wrong);       //0
	private static final int BETWEEN_COLOR=Util.getColor(R.color.password_between);  //1
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < row; j++) {
				points[i][j].draw(canvas, 1);
			}
		}
		ppath.draw(canvas, state);
	}
	...
	private class PWDPath {
		...
		public PWDPath() {
			...
			fPaint = new Paint();
			fPaint.setColor(RIGHT_COLOR);
			fPaint.setStyle(Style.STROKE);

			nPaint = new Paint();
			nPaint.setColor(BETWEEN_COLOR);
		...
		private int getColor(float f) {
			return Color.argb((int) (255 * f), Color.red(BETWEEN_COLOR), Color.green(BETWEEN_COLOR), Color.blue(BETWEEN_COLOR));
		}

		public void setLineWidth(float lineW) {
			nPaint.setStrokeWidth(lineW);
			ePaint.setStrokeWidth(lineW);
			fPaint.setStrokeWidth(lineW);
		 ...
		 */
		public void draw(Canvas c, int s) {
			if (s==1) {
	...
	private class PPoint {
		...
		public PPoint(float x, float y, float r) {
			...
			nPointpPaint.setColor(BETWEEN_COLOR);
			...
			fPointpPaint = new Paint();
			fPointpPaint.setColor(RIGHT_COLOR);
			fPointpPaint.setStyle(Style.FILL_AND_STROKE);
		}

		public void draw(Canvas canvas, int a) {
			if (a==0){
				canvas.drawCircle(x, y, r, ePointpPaint);
			}else if (a==1){
				canvas.drawCircle(x, y, r, nPointpPaint);
			}else {
				canvas.drawCircle(x, y, r, fPointpPaint);
			}
		}
